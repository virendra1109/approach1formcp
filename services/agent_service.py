# """Agent Service - Handles query processing with dynamic server updates"""
...
# class AgentService:
#     """Service for managing the MCP agent with thread support and dynamic updates"""
...
#     async def refresh_orchestrator(self):
#         """
#         Refresh orchestrator with current MCP_SERVERS configuration.
#         Call this after adding/removing servers dynamically.
#         """
#         if not self._initialized:
#             logger.warning("Agent not initialized, cannot refresh orchestrator")
#             return
        
#         logger.info(f"Refreshing orchestrator with servers: {list(MCP_SERVERS.keys())}")
#         await self.agent.orchestrator.update_servers(MCP_SERVERS)
...
#         # Ensure orchestrator knows about all current servers
#         await self.refresh_orchestrator()
        
#         # Get orchestrator plan (now aware of all servers including dynamically added ones)
#         plan,input_token1,output_token1,total_token1 = await self.agent.orchestrator.plan(query, list(self.agent.server_tools.keys()))
...
#                 logger.warning(f"Server '{server_name}' not found in tool indexers")
#                 continue
...
#         # Execute with thread
#         agent_response, updated_thread,input_token,output_token,total_token = await self.agent.orchestrator.execute(
#             query, 
#             all_filtered_tools,
#             thread=thread
#         )
...
#             "token_dict":{
#                 "input (prompt) tokens":input_token1+input_token,
#                 "output (completion) tokens":output_token1+output_token,
#                 "total_tokens":total_token1+total_token
#                         },
#             "result": result,
...
"""Agent Service - Enhanced with better execution flow"""
from typing import Dict, Any, Optional
from utils.agents import MultiMCPAgentWithFiltering
from utils.loggers import logger
from utils.serializers import serialize_agent_response
...
class AgentService:
    ...
    async def process_query(self, query: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Step 1: Get orchestrator plan
        plan, input_token1, output_token1, total_token1 = await self.agent.orchestrator.plan(
            query, 
            list(self.agent.server_tools.keys())
        )
        
        # Step 2: Collect selected tools
        selected_tools = {}
        all_filtered_tools = []
        ...
        # Fallback to all tools if none found
        if not all_filtered_tools:
            logger.warning("No tools found via search, using all tools as fallback")
            for server_name in plan["servers"]:
                if server_name in self.agent.server_tools:
                    all_filtered_tools.extend([
                        t["function"] 
        ...
        logger.info(f"\nTotal tools for execution: {len(all_filtered_tools)}")
        
        # Get or create thread for session
        thread = self.threads.get(session_id) if session_id else None
        
        # Step 3: Execute with thread and plan context
        agent_response, updated_thread, input_token, output_token, total_token = \
            await self.agent.orchestrator.execute(
                query, 
                all_filtered_tools,
                thread=thread,
                plan=plan  # Pass plan for context
            )
